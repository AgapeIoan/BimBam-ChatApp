...
async def test_mark_messages_as_read_resets_unread_and_flags(session_factory, monkeypatch):
    async with session_factory() as db_session:
        ...
        m1 = await msg_repo.create(conversation_id=conv.id, sender_id=b.id, content="hello")
        m2 = await msg_repo.create(conversation_id=conv.id, sender_id=b.id, content="hello 2")
